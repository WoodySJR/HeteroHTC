...
class HGT(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads, num_labels, num_1, num_2, num_3, 
        ...
        self.label_encoding = nn.Parameter(torch.randn(self.num_labels, self.hidden_dim), requires_grad=True)
        ...
        self.PA_c2p = nn.Linear(self.hidden_dim_per_head, self.hidden_dim_per_head, bias=False)
        self.PA_sib = nn.Linear(self.hidden_dim_per_head, self.hidden_dim_per_head, bias=False)
        ## W_phi_message
        self.PM_p2c = nn.Linear(self.hidden_dim_per_head, self.hidden_dim_per_head, bias=False)
        self.PM_c2p = nn.Linear(self.hidden_dim_per_head, self.hidden_dim_per_head, bias=False)
        self.PM_sib = nn.Linear(self.hidden_dim_per_head, self.hidden_dim_per_head, bias=False)
        ## Aggregation
